...
class memes(bot):
    def add(self):
        ...
        self.commonx("INSERT INTO `botMemes` (`keyword`, `url`, `uid`, `time`) VALUES ('{}', '{}', {}, {})".format(self.args[1], self.args[2], self.se.get("user_id"), time.time()))
        self.send("face54[|已添加！|]")
        
    def messageListener(self):
        memesList = self.selectx("SELECT * FROM `botMemes` WHERE `uid`={}".format(self.se.get("user_id")))
        for i in memesList:
    ...
    def rmMemes(self):
        self.commonx("DELETE FROM `botMemes` WHERE `keyword`='{}' and `uid`={}".format(self.args[1], self.se.get("user_id")))
